classdef OFDMComplex < csrd.blocks.physical.modulate.BaseModulator
    ...
    % channel coding, interleaving, scrambling, pilot insertion, and front-end
    % filtering. This implementation provides a complete OFDM transceiver
    % chain suitable for advanced wireless communication systems.
    %
    % The OFDMComplex modulator includes advanced features such as:
    % - Convolutional coding with puncturing for error correction
    % - CRC generation for error detection
    % - Data scrambling for spectral shaping
    % - Symbol and bit interleaving for diversity
    % - Pilot signal insertion for channel estimation
    % - Synchronization and reference signal generation
    % - Front-end filtering for spectral containment
    % - MIMO support with multiple transmit antennas
    %
    % Key Features:
    %   - Complete OFDM physical layer implementation
    %   - Configurable FFT length and bandwidth parameters
    %   - Multiple modulation orders (BPSK, QPSK, 16/64/256/1024-QAM)
    %   - Flexible code rates (1/2, 2/3, 3/4, 5/6)
    %   - MIMO transmission with up to 4 antennas
    ...
    %   - OFDM signal sampling simulation:
    ...
    %
    % Syntax:
    %   ofdmComplexMod = OFDMComplex()
    %   ofdmComplexMod = OFDMComplex('PropertyName', PropertyValue, ...)
    %   modulatedSignal = ofdmComplexMod.step(inputData)
    %
    % Properties:
    %   NumTransmitAntennas - Number of transmit antennas (1, 2, 3, or 4)
    %   BandWidthIndex - Index for bandwidth configuration (positive scalar)
    %   codeRateIndex - Index for code rate selection (positive scalar)
    %   ModulatorType - Modulation type ('psk' or 'qam')
    %   ModulatorOrder - Modulation order (2, 4, 16, 64, 256, 1024)
    %
    % Properties (Access = protected):
    %   sysParam - System parameters structure
    %   txParam - Transmitter parameters structure
    %   txObj - Transmitter objects structure
    %
    % Methods:
    %   setParameters - Configure system and transmitter parameters
    %   txInit - Initialize transmitter objects and filters
    %   baseModulator - Core OFDM modulation with complete PHY processing
    %   genModulatorHandle - Generate configured modulator function handle
    %
    % Example:
    %   % Create advanced OFDM modulator with 64-QAM and 3/4 code rate
    %   ofdmMod = csrd.blocks.physical.modulate.digital.OFDM.OFDMComplex();
    %   ofdmMod.NumTransmitAntennas = 2;
    %   ofdmMod.ModulatorType = 'qam';
    %   ofdmMod.ModulatorOrder = 64;
    %   ofdmMod.BandWidthIndex = 2;    % Bandwidth configuration
    %   ofdmMod.codeRateIndex = 2;     % 3/4 code rate
    %
    %   % Generate modulator handle
    %   modHandle = ofdmMod.genModulatorHandle();
    %
    %   % Create input data
    %   inputData = randi([0 1], 1000, 1); % Random bits
    %
    %   % Modulate the signal (complete PHY processing)
    %   modulatedSignal = ofdmMod.baseModulator(inputData);
    %
    % See also: csrd.blocks.physical.modulate.digital.OFDM.OFDM,
    %           csrd.blocks.physical.modulate.BaseModulator, comm.OFDMModulator

    % Technical References (preserved original comments):
    % https://www.mathworks.com/help/5g/ug/resampling-filter-design-in-ofdm-functions.html
