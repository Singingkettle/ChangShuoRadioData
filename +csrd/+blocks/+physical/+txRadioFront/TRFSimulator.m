        ...
        function resampledSignal = resampleToTarget(obj, inputSignal, inputSampleRate)
            ...
            if relRateDelta <= 1e-12
                ...
                if upsampleFactor > maxFactor || downsampleFactor > maxFactor
                    % Name the RATES, not just the factors. This error tells the
                    % reader to fix the upstream sample-rate plan, so it has to say
                    % which two rates are irreconcilable -- p/q alone cannot be
                    % acted on, and this failure loses a whole scenario. Also state
                    % the smallest divisor that WOULD work, since the planner's
                    % existing repair (snapNarrowSymbolRateToReceiverGrid) works by
                    % putting the modulator rate on an exact receiver submultiple.
                    suggestedDivisor = max(1, round(targetSampleRate / inputSampleRate));
                    error('CSRD:TRF:UnsupportedResampleRatio', ...
                        ['Exact resample ratio %d/%d exceeds the supported ', ...
                         'rational factor limit %d: modulator rate %.10g Hz cannot ', ...
                         'be resampled to receiver rate %.10g Hz. The two share no ', ...
                         'small rational factor. Fix the upstream sample-rate plan ', ...
                         '-- e.g. put the modulator rate on an exact receiver ', ...
                         'submultiple such as %.10g Hz (target/%d).'], ...
                        upsampleFactor, downsampleFactor, maxFactor, ...
                        inputSampleRate, targetSampleRate, ...
                        targetSampleRate / suggestedDivisor, suggestedDivisor);
                end
