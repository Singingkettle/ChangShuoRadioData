...
function measured = buildMeasuredTruth(isolatedSignal, sampleRate, ...
    ...
    % Which estimator produced OccupiedBandwidthHz, and -- when the band could
    % not be resolved from the noise floor at all -- the rejected reading kept
    % for offline estimator analysis only. Never a label.
    sourcePlane.ObwEstimator                  = '';
    sourcePlane.UnresolvedOccupiedBandwidthHz = NaN;
    sourcePlane.SpectralPeakToNoiseFloorDb    = NaN;
    ...
    if ~hasSignal
        ...
        sourcePlane.ObwEstimator = summary.ObwEstimator;
        sourcePlane.SpectralPeakToNoiseFloorDb = summary.SpectralPeakToNoiseFloorDb;
        if strcmpi(summary.MeasurementStatus, 'Unresolved')
            % The waveform carried energy but its occupied bandwidth could not be
            % separated from the noise floor by either estimator. Publishing the
            % rejected reading would hand a downstream regressor a systematically
            % inflated label, so the scalar is nulled and the status says why.
            % The rejected value is retained for estimator analysis.
            sourcePlane.MeasurementStatus = 'Unresolved';
            sourcePlane.OccupiedBandwidthHz = NaN;
            sourcePlane.UnresolvedOccupiedBandwidthHz = ...
                summary.UnresolvedOccupiedBandwidthHz;
        else
...
function fp = computeFramePlaneCache(combinedSignal, sampleRate, observableBwHz)
    ...
    % Every plane must publish an IDENTICAL field set on every path. jsondecode
    % turns a struct array whose members disagree on fields into a cell array,
    % which silently changes the shape every downstream consumer sees, so the
    % unresolved-only field is initialised on all paths.
    fp.ObwEstimator = summary.ObwEstimator;
    fp.SpectralPeakToNoiseFloorDb = summary.SpectralPeakToNoiseFloorDb;
    fp.UnresolvedOccupiedBandwidthHz = NaN;
    if strcmpi(summary.MeasurementStatus, 'Unresolved')
        % Same contract as SourcePlane: the combined frame carried energy but its
        % occupied bandwidth could not be separated from the noise floor. The
        % FramePlane has no SNR field of its own, so a consumer has no way to
        % self-filter here -- which is exactly why the status must say so.
        fp.MeasurementStatus = 'Unresolved';
        fp.OccupiedBandwidthHz = NaN;
        fp.UnresolvedOccupiedBandwidthHz = summary.UnresolvedOccupiedBandwidthHz;
...
function fp = makeEmptyFramePlane()
    ...
    fp = struct( ...
        ...
        'ObwEstimator',         '', ...
        'UnresolvedOccupiedBandwidthHz', NaN, ...
        'SpectralPeakToNoiseFloorDb',    NaN);
end
