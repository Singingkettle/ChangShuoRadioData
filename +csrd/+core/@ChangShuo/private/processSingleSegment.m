function modulatedSignalSegment = processSingleSegment(obj, FrameId, currentTxScenario, currentTxId, segIdx)
    ...
    if ~isempty(modulatedSignalSegment)
        ...
        % Message source plane (analog -> Audio, digital -> RandomBit). The
        % source is recorded so the dataset can verify that each annotated
        % radio event was driven by the physically correct baseband.
        if isfield(currentTxScenario, 'Message') && isstruct(currentTxScenario.Message) && ...
                isfield(currentTxScenario.Message, 'Type') && ...
                ~isempty(currentTxScenario.Message.Type)
            modulatedSignalSegment.Planned.MessageSource = ...
                char(string(currentTxScenario.Message.Type));
        elseif isfield(currentTxScenario, 'Message') && isstruct(currentTxScenario.Message) && ...
                isfield(currentTxScenario.Message, 'TypeID') && ...
                ~isempty(currentTxScenario.Message.TypeID)
            modulatedSignalSegment.Planned.MessageSource = ...
                char(string(currentTxScenario.Message.TypeID));
        else
            modulatedSignalSegment.Planned.MessageSource = '';
        end
        if ~isempty(modulatedSignalSegment.Planned.ModulationFamily)
            modulatedSignalSegment.Planned.IsDigital = ...
                ~csrd.support.modulation.isAnalogModulationFamily( ...
                    modulatedSignalSegment.Planned.ModulationFamily);
        else
            modulatedSignalSegment.Planned.IsDigital = true;
        end
