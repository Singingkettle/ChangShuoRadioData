classdef (StrictDefaults)BaseOQPSK < comm.internal.OQPSKBase
    ...
    %   via filtering.
    %
    %   MOD = comm.OQPSKModulator(DEMOD) creates an OQPSK modulate
    %   object, MOD, with symmetric configuration to the OQPSK demodulator
    %   object DEMOD.
    %
    %   MOD = comm.OQPSKModulator(Name,Value) creates an OQPSK modulate
    ...
    %
    %   MOD = comm.OQPSKModulator(PHASE,Name,Value) creates an OQPSK modulate
    ...
    %   OQPSK modulate object, MOD, and returns baseband modulated output,
    ...
    %   sps = 12;
    %   rate = 4e6;
    %   modulate = comm.OQPSKModulator('BitInput', true, 'PulseShape', 'Half sine', ...
    %                                   'SamplesPerSymbol', sps, 'SymbolMapping', [3 2 0 1]);
    %   waveform = modulate(bits);     % joint oqpsk modulation and filtering
    ...
    %   sps = 12;
    %   rate = 4e6;
    %   modulate = comm.OQPSKModulator('PulseShape', 'Normal raised cosine', 'RolloffFactor', 0.8, ...
    %                                   'SamplesPerSymbol', sps, 'SymbolMapping', [3 2 0 1]);
    %   waveform = modulate(symbols);     % joint oqpsk modulation and filtering
    ...
    %   modulate = comm.OQPSKModulator('BitInput', true, 'SamplesPerSymbol', sps, 'PulseShape', 'Root raised cosine');
    %   oqpskWaveform = modulate(bits);
    %
    %   % Channel:
    %   snr = 0;
    ...
    %
    %   % Receiver
    %   demodulator = comm.OQPSKDemodulator(modulate);
    %   demodulated = demodulator(received);
    %
    %   % Compute BER:
    %   delay = (1+modulate.BitInput)*modulate.FilterSpanInSymbols;
