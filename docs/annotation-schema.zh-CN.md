...
`Execution.AppliedSNRdB` 只在统计信道(AWGN/Rayleigh/Rician)上是受控目标
(默认 [-10, 30] dB),那里信道工厂按它定噪。**射线追踪不走这套实现**:它施加
所追踪几何(无路径时为自由空间回退)的物理路径损耗,所以射线追踪发射机实测的
`SourcePlane.SNRdB` 由几何涌生、**下不封底**——遮挡 NLOS 或全阻断链路合法地到达
时低于帧噪声底 100+ dB。射线追踪源仍记录 `AppliedSNRdB`,但那不是实测 SNR;实测
值是 `SourcePlane.SNRdB`。

### 可检测性:该源在被保存的帧里是否可被找到

由于测量平面是注噪前逐发射机测的,即使某个源在交付的含噪帧里远低于噪声底(按上文
射线追踪下很常见),它的 OBW/中心/族标签仍然干净自信。因此 `SourcePlane` 携带一个
可检测性判定,使消费方不会在"保存帧里其实是纯噪声"的地方训练"这里有信号":

| 字段 | 含义 |
|-------|---------|
| `Detectable` | 逻辑值;`SNRdB >= DetectabilityThresholdDb` |
| `DetectabilityStatus` | `Detectable` \| `BelowNoiseFloor` \| `NoSignal` |
| `DetectabilityThresholdDb` | 施加的门限(dB),默认 −30 |

−30 dB 门限不是某个具体检测器的 SNR 壁(那样会武断):在 −30 dB 源只贡献噪声功率的
0.1%,低于能量检测器和实用特征检测器的单帧 SNR 壁,所以它标记的是任何检测器在单帧
内都找不到、因而标签不可信的那些源。门限随数据走,读者能看到用的是哪条线;想要更紧、
针对特定检测器门限的消费方可从必需的 `SNRdB` 重新派生。COCO 转换器对几何可见但
`BelowNoiseFloor` 的源用 `skip_reason = below_noise_floor` 跳过,与 `not_visible`
(几何)、`measurement_status_*`(无信号可测)相区分。这三个可检测性字段是**加法式**
的——不改任何现有测量字段的含义,所以测量契约版本不变,早于这些字段写出的标注仍可
读取(闸门/转换器回退到从 `SNRdB` 派生可检测性)。
