...
| H3 | `+csrd/+pipeline/+scenario/checkTransmissionInterval.m`:58-67；`+csrd/+core/@ChangShuo/private/processSingleTransmitter.m`:47-54 | 帧时间落在第一个匹配区间即返回，`NumSegments=1`——**当前代码不支持"一帧多 burst"**；蓝图若描述多 burst，执行端将丢失除第一段之外的所有 burst | 规划缺失 + 字段错位 | Phase 2 |
| H4 | `+csrd/+pipeline/+core/applyAntennaConfigFromSegments.m`:37-68 | 用**最后一段**非空 segment 的 `NumTransmitAntennas` 覆盖 `TxInfo.NumTransmitAntennas` 与 `SiteConfig.Antenna`；多段 burst 时取最后一段而不是图纸值 | 执行覆盖 | Phase 1（删整段） |
...
|----|-----------|----------|----------|----------|-------|
| D1 | `+csrd/+pipeline/+core/applyAntennaConfigFromSegments.m` 整文件 | 天线数图纸定，不再回写 | 删除调用点 `processSingleTransmitter.m`:96-104；蓝图字段 `HardwarePlan.NumTransmitAntennas` 直传 `TxInfo` | H4 | Phase 1 |
...
| D6 | `+csrd/+factories/ChannelFactory.m`:86-140 `ChannelBlockStepFailed` 软化输出 | 把异常包成静默错误向下游传 | 仅允许 `csrd.pipeline.scenario.isScenarioSkipException` 通过；其它异常直接 SkipScenario，不再写半截信号 | H11 | Phase 2 |
...
|----|-----------|----------|----------------|
| K1 | RayTracing `NoValidPaths` / `NoBuildingData` 整场景跳过 | 已通过 `csrd.pipeline.scenario.isScenarioSkipException` 成型；这是合理的不可施工标记 | 必须在 `Execution.Errors[]` 留下记录，便于统计 |
...
| K3 | 噪声带宽 clamp（`csrd.pipeline.linkbudget.resolveNoiseBandwidth`，已上线） | 物理上正确 | 必须把 clamp 后的 `NoiseBandwidthHz` 写入 `Execution.LinkBudget` |
...
实施约束：
- 文件路径强制 `+csrd/+pipeline/+blueprint/`，**不在根、不在 factories**（违反 `csrd-workflow.mdc` rule 5）
- 纯静态方法，不持状态；调用方在 `SimulationRunner` 与单元测试中复用
- Reject 与 Warn 同时返回；调用方决定 reject 是否触发 SkipBlueprint
- 复用 `csrd.pipeline.scenario.isScenarioSkipException` 一致的命名（不抛异常，靠 `ok==false` 触发上层 SkipBlueprint）
...
每个画像在 `+csrd/+catalog/+profile/<ProfileName>.m` 内定义为函数返回 struct，配置文件**只引用画像名**。
...
矩阵在 `+csrd/+catalog/+profile/AntennaModulationMatrix.m` 中以 `containers.Map` 形式定义，validator `ModulationAntennaCompatible` check 直接查表。
...
**阶段退出条件（v0.4 修订）**：
- 7 个频段画像 + 3 个接收机画像入库 `+csrd/+catalog/+profile/`
...
- `csrd.pipeline.annotation.makeMeasurementRecord(...)` 是唯一入口，禁止其它代码直接 `struct(...)` 拼 annotation
- `SchemaVersion` 字段由 `makeMeasurementRecord` 内部硬编码，**不依赖配置**
- LegacyV1Aliases 由专门的 `csrd.pipeline.annotation.appendLegacyAliases` 函数生成；移除该兼容期时，**只删一个调用点**
...
| 路径 | 用途 |
|------|------|
| `+csrd/+pipeline/+blueprint/` | `BlueprintFeasibilityValidator.m`，验图层入口 |
| `+csrd/+catalog/+profile/` | 频段画像、接收机画像、天线-调制矩阵 |
| `+csrd/+utils/+measure/` | `measureSourcePlane.m`、`measureFramePlane.m`、`detectBurstEnvelope.m` 等测量函数 |
| `+csrd/+pipeline/+annotation/` | `makeMeasurementRecord.m`、`appendLegacyAliases.m` |
...
| 路径 | 处置 | Phase |
|------|------|-------|
| `+csrd/+pipeline/+core/applyAntennaConfigFromSegments.m` | 删除 | 1 |
...
| 旧测试运行产物目录 | 迁到 `artifacts/tests/runs/` | 0→1 |
...
#### Q12. §13.3 关于旧测试运行产物目录迁到 `artifacts/tests/runs/` 的治理建议，是否足够贴当前仓库？
...
| A2 | H3 | "checkTransmissionInterval 一帧只判一个 interval" | 不准确（指错文件） | `+csrd/+pipeline/+scenario/checkTransmissionInterval.m` 本身可以返回多 interval；真正限制发生在 `+csrd/+blocks/+scenario/@CommunicationBehaviorSimulator/private/calculateTransmissionState.m`:34-69，对 Burst/Scheduled/Random 模式强制把 `CurrentIntervalIdx` 收缩为单值；下游 `+csrd/+core/@ChangShuo/private/processSingleTransmitter.m`:48-53 再据此走 `NumSegments=1` 分支 |
...
| H16 | `+csrd/+catalog/+profile/` 目录**不存在**（grep 无结果）；v0.3 §5.bis 给出的 7 个频段画像 + 3 个接收机画像目前仅是文档表格，没有任何 `.m` 文件；BlueprintFeasibilityValidator 类同样未实现（grep `BlueprintFeasibilityValidator|computeBlueprintHash|profileLoader` 在 `+csrd` 内 0 命中）| `find +csrd/+catalog/+profile/` 不存在；`Grep BlueprintFeasibilityValidator` 0 命中 | v0.3 §4.bis 12 条 check / §5.bis 画像数值表全部"纸面状态"，下游 ScenarioFactory 仍按旧自由参数采样，蓝图爆炸风险未缓解 | Phase 2 全骨架落地 |
| H17 | `MeasuredTruth.*` 字段**0 实现**：grep `MeasuredTruth\|SourcePlane\|FramePlane` 在 `+csrd` 仅命中文档/注释，无任何测量代码；`buildSourceAnnotation` 全文不调用 `obw / spectrumCentroid / detectBurstEnvelope`；当前最终 annotation 中的"带宽 GT"实际仍是 `Realized.Bandwidth`（即调制器 obw），不是 receiver-view 测量结果 | `+csrd/+core/@ChangShuo/private/processReceiverProcessing.m` 全文 grep `obw\|MeasuredTruth` 0 命中 | v0.3 §3.1.bis D 表 "真正的 MeasuredTruth" 全部为空，下游训练拿到的标签实际仍是 ExecutionTruth | Phase 4 新增 `+csrd/+pipeline/+measurement/` 包 |
...
#### 16.8.1 `+csrd/+catalog/+profile/` 目录骨架
...
各档直接绑定到 `comm.PhaseNoise` 的 `(Level, FrequencyOffset)` 参数对，由 `+csrd/+catalog/+profile/+phaseNoise/<Level>.m` 返回。
...
落地工具：`+csrd/+pipeline/+annotation/sanitizeForJson.m`（Phase 0 新增）。
...
- [x] 已 grep `+csrd/+catalog/+profile/` 不存在；grep `BlueprintFeasibilityValidator|computeBlueprintHash` 在 `+csrd` 内 0 命中，确认 §16.2 H16 / H17 事实
...
| 主要新增 | `+csrd/+runtime/+toolbox/validateRequiredToolboxes.m`<br/>`+csrd/+runtime/+logger/+policy/LogPolicy.m`<br/>`+csrd/+pipeline/+annotation/sanitizeForJson.m`<br/>`tests/regression/test_baseline_sweep_200.m`<br/>`tests/regression/baseline_recipe_v0.m`<br/>`tests/regression/test_simulation_runner_startup_hooks.m`<br/>`tests/regression/Phase0FakeEngine.m`<br/>`tests/unit/{ValidateRequiredToolboxes,LogPolicyDev,LogPolicyLargeMC,SanitizeForJson{Basic,Recursive,ComplexAllowlist}}Test.m`<br/>`docs/baselines/2026-04-baseline-v0.json` |
...
| 主要新增 | `+csrd/+catalog/+profile/` 15 个 .m（profileLoader + 7 bands + 3 receivers + 3 phaseNoise + 1 antennaCompat）<br/>`+csrd/+pipeline/+blueprint/computeBlueprintHash.m`（含 6 个 local `canonicalize*` helpers）<br/>`+csrd/+pipeline/+blueprint/BlueprintFeasibilityValidator.m`（21 条静态 check + ValidationReport 由 `validate()` 直接构造）|
| 主要修改 | `+csrd/+factories/ScenarioFactory.m` 加 generate→validate→resample 循环（最多 8 次，仅 frame=1）+ 3 个 public read-only properties；C-1 silent fallback fail-fast 化<br/>`+csrd/+factories/ChannelFactory.m` 删 D5 `modelNames{1}` 兜底 + 加 Hidden static helper<br/>`+csrd/+blocks/+scenario/@CommunicationBehaviorSimulator/private/performScenarioFrequencyAllocation.m` 删 D7 strategy switch + warning fallback<br/>`+csrd/+core/@ChangShuo/ChangShuo.m` 加 Hidden public `getScenarioBlueprintProvenance()`<br/>`+csrd/SimulationRunner.m` 调 provenance 写 `Header.Runtime.{BlueprintHash, BlueprintResamples, ValidatorVersion}`<br/>`+csrd/+pipeline/+scenario/isScenarioSkipException.m` 加 `CSRD:Blueprint:Unsamplable` 白名单<br/>`tests/regression/baseline_recipe_v0.m` 所有 cohort `RxRange=[1,1]`（Phase 3 待解除，详见 phase-2-blueprint.md §9.4 P3-followup-1）|
...
| 项目 | 值 |
|------|----|
| 状态 | **Frozen**（2026-04-28：S1-S8 已完成；Phase 6 Frozen，release readiness 内容门禁已落地） |
...
| 主要范围 | release checklist（`tools/release/run_csrd_release_readiness.m` 已落地） / annotation v2 schema validation（`readAnnotationV2` 已落地） / COCO v2 converter（S5 已落地） / performance diagnostic report（S6 已落地） / local CI readiness（S7 已落地） / release freeze documentation gate（S8 已落地） |
