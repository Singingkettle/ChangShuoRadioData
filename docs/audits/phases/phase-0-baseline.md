...
- `+csrd/+utils/+toolbox/`（新增）
- `+csrd/+utils/+logger/+policy/`（新增）
- `+csrd/+utils/+annotation/`（新增）
...
csrd.utils.toolbox.validateRequiredToolboxes( ...
...
% LOGPOLICY  Centralized log-level policy for csrd.utils.logger.
%
%   policy = csrd.utils.logger.policy.LogPolicy(level)
%
% LEVEL options:
...
%
% Methods:
%   apply()        - mutate the singleton csrd.utils.logger.GlobalLogManager
...
```matlab
policy = csrd.utils.logger.policy.LogPolicy( ...
...
%
%   [clean, manifest] = csrd.utils.annotation.sanitizeForJson(value)
%   [clean, manifest] = csrd.utils.annotation.sanitizeForJson(value, options)
%
...
- `NumScenarioSkipped` / `NumScenarioSkippedByReason`（按 `csrd.utils.scenario.isScenarioSkipException` 识别的 reason 分桶）
...
function setupImpl(obj)
    ...
    csrd.utils.toolbox.validateRequiredToolboxes(toolboxLevel);
...
policy = csrd.utils.logger.policy.LogPolicy(logPolicyLevel);
policy.apply();
...
try
    ...
    [cleanAnnotation, sanitizeManifest] = csrd.utils.annotation.sanitizeForJson( ...
        scenarioAnnotation);
...
| R7 | `+csrd/+utils/+annotation/sanitizeForJson.m` 与未来 Phase 4 测量层重写 annotation 写盘逻辑冲突 | Phase 0 不动 `processReceiverProcessing.m`；sanitize 仅在 SimulationRunner 写盘前调用一次，Phase 4 重写时只需保证最后仍调一次 sanitize |
...
- [x] **C1** `+csrd/+utils/+toolbox/validateRequiredToolboxes.m` 已落地，单元测试 6.1 三档全过；在 R2025a 主开发机上跑 `'minimal'` 不报错
- [x] **C2** `+csrd/+utils/+logger/+policy/LogPolicy.m` 已落地；`'LargeMC'` 模式下对 50 个连续 debug 调用，rolling 文件命中数为 0（`LogPolicyLargeMCTest.fiftyDebugCallsLeaveLogFileUntouched`，强于原文写的 console 0 / file 50：实测 LargeMC 把 file 阈值抬到 INFO 后 debug 不再落盘）
- [x] **C3** `+csrd/+utils/+annotation/sanitizeForJson.m` 已落地，3 个 `*Test` 文件 26 个 cases 全过
...
| 路径 | 类型 | 行数估计 |
|------|------|----------|
| `+csrd/+utils/+toolbox/validateRequiredToolboxes.m` | 函数 | ~120 |
| `+csrd/+utils/+logger/+policy/LogPolicy.m` | 类 | ~180 |
| `+csrd/+utils/+annotation/sanitizeForJson.m` | 函数 | ~220 |
