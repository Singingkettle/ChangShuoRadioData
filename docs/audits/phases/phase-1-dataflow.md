...
```60:68:+csrd/+utils/+scenario/checkTransmissionInterval.m
...
##### Step 1：`+csrd/+utils/+scenario/checkTransmissionInterval.m`
...
case 'Burst'
    if isfield(pattern, 'Intervals') && ~isempty(pattern.Intervals)
        overlaps = csrd.utils.scenario.findOverlappingTransmissionIntervals(frameId, pattern);
        if isempty(overlaps)
...
> 选择 fail-fast 而非 warning，是因为 §17.3 把 H9 列在 "解决条目" 而不是 "deprecation"。错误标识符均纳入 `csrd.utils.scenario.isScenarioSkipException` 白名单要在 §3.3.3 决策。
...
实施时改 `+csrd/+utils/+scenario/isScenarioSkipException.m`，在白名单加 `CSRD:Scenario:EmptyEntities`。
...
function seed = deriveChannelSeed(~, txIdStr, rxIdStr, channelLinkInfo, frameId)
    ...
    digest = csrd.utils.hash.shortInt32Hash(payload);  % 新工具，见 §4.3
...
放在 `+csrd/+utils/+contract/assertSignalStructContract.m`。**Phase 1 不在生产代码 Always-on**——只在 ContractTest 内 invoke，否则会大量 throw 让 baseline 退化。生产代码仅在 H14 / RFImpairments 缺失时 throw（§3.5、§3.6.2 已含）。
...
| `+csrd/+utils/+scenario/isScenarioSkipException.m` | §3.3.3 | +1 行（白名单加 EmptyEntities）|

### 4.2 新增文件（共 3 个）

| 文件 | 用途 |
|------|------|
| `+csrd/+utils/+scenario/findOverlappingTransmissionIntervals.m` | §3.2 多 burst 收集 |
| `+csrd/+utils/+contract/assertSignalStructContract.m` | §3.6 契约断言 helper |
| `+csrd/+utils/+hash/shortInt32Hash.m` | §3.4 BurstId-aware seed 用，见 §4.3 |
...
- `+csrd/+utils/+annotation/sanitizeForJson.m`（Phase 0 已 Frozen）
