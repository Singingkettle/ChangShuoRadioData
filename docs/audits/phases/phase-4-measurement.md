...
| 编号 | 标题 | 当前问题 / 现状 | 处方 |
|------|------|----------------|------|
| **P4-1** | 测量包不存在 | `+csrd/+utils/+measure*/` glob 0 命中；`+csrd/+core/@ChangShuo/private/processReceiverProcessing.m` 全文 grep `obw\|MeasuredTruth` 0 命中；现 annotation 中 `Realized.Bandwidth` 实际是调制器对**调制后基带**调用 `obw` 的结果（H2/H17）—— 不是 receiver-view 测量 | §3.1（新 `+csrd/+utils/+measurement/` 包 5 函数）|
...
### 3.1 P4-1 `+csrd/+utils/+measurement/` 测量包（5 函数）
...
framePlaneCache = struct( ...
    'OccupiedBandwidthHz', csrd.utils.measurement.obwActual( ...
        combinedSignal.Signal, sampleRate, 99), ...
    'CenterFrequencyHz',   csrd.utils.measurement.spectrumCentroid( ...
        combinedSignal.Signal, sampleRate), ...
    'TimeOccupancy',       envelopeInfo.TimeOccupancy, ...
    'FrequencyOccupancy',  csrd.utils.measurement.frequencyOccupancy( ...
...
| `Spatial.*` | `Truth.Execution.GeometrySnapshot.{TxPositionM,RxPositionM,LinkDistanceM}` | 同上 |
...
| 路径 | 说明 |
|------|------|
| `+csrd/+utils/+measurement/obwActual.m` | OBW 包装 |
| `+csrd/+utils/+measurement/spectrumCentroid.m` | 频谱质心 |
| `+csrd/+utils/+measurement/actualSnrFromComponents.m` | 显式功率 SNR |
| `+csrd/+utils/+measurement/detectBurstEnvelope.m` | 滑窗能量检测 |
| `+csrd/+utils/+measurement/frequencyOccupancy.m` | 占用比例 |
...
| `+csrd/+utils/+blueprint/BlueprintFeasibilityValidator.m` | 实化 `checkMeasurementCompleteness` / `checkDopplerSelfConsistency` / `checkOverlapAnnotationConsistent` 三 stub |
| `+csrd/+factories/ChannelFactory.m` | channel block 创建时根据 channel 类型置位 `channelOutput.ChannelInfo.HasInternalDoppler` |
| `+csrd/+utils/+scenario/isScenarioSkipException.m` | 白名单加 `CSRD:Annotation:` 与 `CSRD:Measurement:` token |
...
|------|------|--------------------|------|
| **S1** | `+csrd/+utils/+measurement/` 包 5 函数 + `MeasurementPackageTest` | unit test 全过 | — |
...
| 路径 | 说明 |
|------|------|
| `+csrd/+utils/+measurement/obwActual.m` | OBW 包装；2026-04-26 改 **peak-relative thresholding**（默认 `PeakRelativeDb=-3 dBc`）替代原 noise-floor 去噪法，使 clean / noisy 两端 OBW 在 SNR ≥ 6 dB 时收敛到同一阈值（C8 决定项）|
| `+csrd/+utils/+measurement/spectrumCentroid.m` | 频谱质心 `argmax∫|FFT|² · f / ∫|FFT|²`，单位 Hz |
| `+csrd/+utils/+measurement/actualSnrFromComponents.m` | 显式功率 → SNR `10·log10(Ps/Pn)`，`Pn ≤ 0` throw |
| `+csrd/+utils/+measurement/detectBurstEnvelope.m` | 滑窗能量检测，输出 TimeOccupancy / NumBursts |
| `+csrd/+utils/+measurement/frequencyOccupancy.m` | 占用比例 `obw / observable`，clip 到 `[0,1]`，分母 = 0 返回 NaN |
...
| `+csrd/+utils/+blueprint/BlueprintFeasibilityValidator.m` | `checkMeasurementCompleteness` 改 PASS（enforce 已迁到 saveScenarioData hook）；`checkDopplerSelfConsistency` 实化（任 emitter `MaxSpeedMps>0` 时 `MeasurementPolicy.RequireDopplerShiftHz` 必须 true）；`checkOverlapAnnotationConsistent` 实化（`length(BurstSchedule.Bursts) == length(unique(ActiveIntervalIndices))`）|
...
| `+csrd/+utils/+scenario/isScenarioSkipException.m` | 白名单加 `CSRD:Annotation:` 与 `CSRD:Measurement:` token，避免误杀 |
...
| 旧 `RealizedVsPlannedBwAbsRelDiffP95` metric | `test_baseline_sweep_200.m::localScoreSource` | 替换为 `ExecutionVsMeasuredBwAbsRelDiffP95` |
| `obwActual` 内 `NoiseFloorPercentile` / `NoiseFloorMargin` / `computeDenoisedObw` 旧实现 | `+csrd/+utils/+measurement/obwActual.m` | clean/noisy 两端阈值不一致导致 RRC QPSK 在低 SNR 下 OBW 偏差 12%；改 peak-relative 后差异在 SNR ≥ 6 dB 内 < 5% |
