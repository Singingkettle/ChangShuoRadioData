...
OFDM_signal_TTI=zeros(1,Num_symbol*Size_FFT*5/4);

for c_symbol=1:Num_symbol
    x=zeros(1,Size_FFT);
    for c_subcarrier=Index_Fist_Subcarrier:Index_Fist_Subcarrier+Num_date_persymbol-1
        x(c_subcarrier)=x_input(c_subcarrier-Index_Fist_Subcarrier+1,c_symbol);
    end
    
    Stx=ifft(x)*sqrt(Size_FFT);
    Stx=[Stx(Size_FFT*3/4+1:Size_FFT),Stx];
    OFDM_signal_TTI((c_symbol-1)*Size_FFT*5/4+1:c_symbol*Size_FFT*5/4)=Stx;
end
