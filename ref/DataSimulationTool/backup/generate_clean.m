...
channel_clean = MyModClassTestChannel(...
                        'channel_type', 'clean', ...
                        'SampleRate', fs, ...
                        'SNR', SNR, ...
...
for i=1:10000
%     s = clock;
    fprintf('Under clean channel, generate data of number %05d.\n', i);
    ...
    if ~isempty(y)
        ...
        fid = fopen(sprintf("./v1/anno/%06d.json", i),'w');
        fprintf(fid, s); 
        fclose(fid);
    
        save(sprintf("./v1/sequence_data/iq/%06d.mat", i),  'signal_data');
    end
end