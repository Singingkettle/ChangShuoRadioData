...
for s_index=1:7
    ...
    for i=1:10000
    %     s = clock;
        fprintf('Under rician channel, generate data of number %05d.\n', i);
        ...
        if ~isempty(y)
            ...
            fid = fopen(sprintf("./v%d/anno/%06d.json", s_index+8, i),'w');
            fprintf(fid, s); 
            fclose(fid);
        
            save(sprintf("./v%d/sequence_data/iq/%06d.mat", s_index+8, i),  'signal_data');
        end
    end
end