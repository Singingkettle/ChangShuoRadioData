classdef CalculateTransmissionStateTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function frameDurationOverridesEverything(testCase)
            ...
            [active, idx, s, e] = csrd.pipeline.scenario.checkTransmissionInterval(6, pattern);
            ...
            [active, ~, ~, ~] = csrd.pipeline.scenario.checkTransmissionInterval(11, pattern);
            testCase.verifyFalse(active);

            % frame 16 -> t = 1.5s -> active in interval 2
            [active, idx, s, e] = csrd.pipeline.scenario.checkTransmissionInterval(16, pattern);
            testCase.verifyTrue(active);
        ...
        function fallbackUsesObservationDurationAndNumFrames(testCase)
            ...
            [active, idx] = csrd.pipeline.scenario.checkTransmissionInterval(5, pattern);
            testCase.verifyTrue(active);
            testCase.verifyEqual(idx, 1);

            % frame 25 -> t = 2.4s -> active in interval 2
            [active, idx] = csrd.pipeline.scenario.checkTransmissionInterval(25, pattern);
            testCase.verifyTrue(active);
            testCase.verifyEqual(idx, 2);
        end
        ...
        function emptyIntervalsReturnsInactive(testCase)
            ...
            [active, idx] = csrd.pipeline.scenario.checkTransmissionInterval(1, pattern);
            testCase.verifyFalse(active);
            testCase.verifyEqual(idx, 0);
        end
