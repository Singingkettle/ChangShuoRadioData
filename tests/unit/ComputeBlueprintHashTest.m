classdef ComputeBlueprintHashTest < matlab.unittest.TestCase
    ...
    %csrd.utils.blueprint.computeBlueprintHash.
    %
    ...
    methods (Test)
        ...
        function roundTripBytewiseEqual(testCase)
            ...
            h1 = csrd.utils.blueprint.computeBlueprintHash(bp);
            h2 = csrd.utils.blueprint.computeBlueprintHash(bp);
        ...
        function fieldOrderIsInvariant(testCase)
            ...
            h1 = csrd.utils.blueprint.computeBlueprintHash(s1);
            h2 = csrd.utils.blueprint.computeBlueprintHash(s2);
            testCase.verifyEqual(h2, h1);
        end
        ...
        function cellVsVectorDistinguished(testCase)
            ...
            % collapsed.
            h1 = csrd.utils.blueprint.computeBlueprintHash({1, 2, 3});
            h2 = csrd.utils.blueprint.computeBlueprintHash([1 2 3]);
            testCase.verifyNotEqual(h2, h1);
        end
        ...
        function nanThrowsHashFailed(testCase)
            ...
            testCase.verifyError( ...
                @() csrd.utils.blueprint.computeBlueprintHash(bp), ...
        ...
        function logicalAndIntegerAreDistinguished(testCase)
            ...
            h1 = csrd.utils.blueprint.computeBlueprintHash(true);
            h2 = csrd.utils.blueprint.computeBlueprintHash(1);
            testCase.verifyNotEqual(h2, h1);
        end

        function singleAndDoubleAreDistinguished(testCase)
            h1 = csrd.utils.blueprint.computeBlueprintHash(single(1.5));
            h2 = csrd.utils.blueprint.computeBlueprintHash(1.5);
            testCase.verifyNotEqual(h2, h1);
        end

        function emptyArrayHasStableHash(testCase)
            h1 = csrd.utils.blueprint.computeBlueprintHash([]);
            h2 = csrd.utils.blueprint.computeBlueprintHash([]);
            testCase.verifyEqual(h2, h1);
            % And it should differ from an empty cell:
            h3 = csrd.utils.blueprint.computeBlueprintHash({});
            testCase.verifyNotEqual(h3, h1);
        end

        function structArrayHashIsStable(testCase)
            sa = struct('x', {1, 2, 3}, 'y', {'a','b','c'});
            h1 = csrd.utils.blueprint.computeBlueprintHash(sa);
            h2 = csrd.utils.blueprint.computeBlueprintHash(sa);
            testCase.verifyEqual(h2, h1);
        end
