classdef ScenarioPlanAnnotationContractTest < matlab.unittest.TestCase
    ...
    methods (Test)
        function annotationHeaderIncludesScenarioPlanFrame(testCase)
            ...
            testCase.verifyTrue(isfield(frameAnn, 'FramePlan'));
            testCase.verifyEqual( ...
                frameAnn.ScenarioPlan.Frame.FrameNumSamples, 1024);
            testCase.verifyEqual(frameAnn.FramePlan.FrameWindowSec, ...
                [0, 1024 / 50e6], AbsTol=1e-12);
            ...
            testCase.assertGreaterThanOrEqual(numel(frameAnn.SignalSources), 1);
            source = frameAnn.SignalSources(1);
            testCase.verifyEqual( ...
                source.Truth.Design.GeometryEvaluationPolicy, ...
                'SegmentMidpoint');
            testCase.verifyEqual( ...
                source.Truth.Execution.GeometrySnapshot.EvaluationPolicy, ...
                'SegmentMidpoint');
        end
    end
end
