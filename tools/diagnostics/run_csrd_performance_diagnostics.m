...
obwPath = fullfile(projectRoot, '+csrd', '+pipeline', '+measurement', ...
    'obwActual.m');
% The occupied-bandwidth kernel (and therefore the pwelch dependency) lives in the
...
hotspots.Items = [
    ...
    makeHotspot('obwActualItuDefault', ...
        contains(obwCode, '''Method'', ''itu-99'''), obwPath, ...
        ['Occupied bandwidth must default to the ITU-R SM.328 99%-power ', ...
         'definition; an x-dB-down width is a different quantity.'])
